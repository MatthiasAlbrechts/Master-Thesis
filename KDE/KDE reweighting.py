...
# calculate arithmetic mean with respective confidence interval
high_level_FESs = np.stack(FES_list)
mean_high_level_FES = np.mean(high_level_FESs, axis=0)
confidence_interval = mean_high_level_FES + tcrit * np.std(high_level_FESs, axis=0) / np.sqrt(n_subsets)
